...
- `in_use[]` - why a tool is in use, read from `herdr agent list`, `no-mistakes runs`, and the process table (`/proc/<pid>/exe`), never from Firstmate's files. When several rows of one surface are in use for the same facts (every claude plugin runs through the claude binary), the surface's manager row carries the detail and the other rows read `same as <surface>,<tool>`; a row whose manager row is not in the output keeps its full detail. `apply` refuses an in-use tool with this reason.
...
**The session-start hook (primary).** `upkeep-axi setup hooks` registers a `SessionStart` hook with Claude Code (`~/.claude/settings.json`), Codex (`~/.codex/hooks.json`, plus the `[features].hooks = true` flag in `~/.codex/config.toml`), and OpenCode (a managed plugin under `~/.config/opencode/plugins/`), so every agent session starts with the upkeep-axi dashboard already in context. The hook runs the bounded `ambient` view - known gaps only (an in-use row without a gap stays in the counts, off the row list), in-use gaps first then severity, capped at a few lines with the counts pre-computed - never the full inventory; deep data is one `upkeep-axi status` away. The view probes nothing: it reads the inventory the last unfiltered `upkeep-axi status` run saved under `$XDG_STATE_HOME/upkeep-axi/status.json`, brought up to date with applies journaled since, so a session starts in well under the hook's 10 s budget even offline. Until a status run has happened it says so and names the command; once the saved inventory is older than a day it says that too. Setup is idempotent, repairs the command after the tool moves, and only ever touches entries carrying the `upkeep-axi` marker. `upkeep-axi setup hooks --status` reports what is installed without writing; `upkeep-axi ambient` previews exactly what the hook injects.
...
All seventeen version-one surfaces have shipped; the tool runs on this WSL2 Ubuntu host only:
...
| `snap`        | user-launchable app snaps, report-only  | snapd REST API over `/run/snapd.socket` (`/v2/snaps`)    | `/v2/find?select=refresh` - the candidate snapd offers this host; apply is the exact `sudo snap refresh <name>` text |
...
The snap surface lists only active app snaps with at least one non-daemon app (a configured `surfaces.snap.tools[]` name overrides the filter). `latest` comes only from the refresh candidate snapd offers this host on the tracked channel, so a phased or held release reports no gap; a candidate whose version parses forward takes the semver tier, one proved only by a differing revision tiers `major`. Holds and refresh inhibition are carried as vendor facts, never errors.
